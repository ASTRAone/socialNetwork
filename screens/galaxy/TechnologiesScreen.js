import React, {useEffect, useState, useCallback} from 'react';
import {
  View,
  Text,
  StyleSheet,
  FlatList,
  Image,
  TouchableOpacity,
  RefreshControl,
} from 'react-native';

import { fetchTechnologies } from '../../actions/technologies.action';
import {useDispatch, useSelector} from 'react-redux';
import {useNavigation} from '@react-navigation/native';
import {useTheme} from '@react-navigation/native';

import R from '../../resources/R';
import {unwrapResult} from '@reduxjs/toolkit';

import {UnexpectedError} from '../../components/UnexpectedError';
import {renderImages} from '../../utility/renderSrcImages';
import {white} from '../../utility/darkTheme';

const TechnologiesScreenItem = ({model, crew, length}) => {
  const navigation = useNavigation();

  return (
    <TouchableOpacity
      style={styles.itemContent}
      onPress={() => navigation.navigate('TechnologiesCardScreen', {model})}>
      <Image
        style={{
          width: 180,
          height: 150,
        }}
        source={renderImages(model)}
      />
      <View style={styles.itemContentText}>
        <Text
          numberOfLines={1}
          style={[styles.itemText, {color: R.colors.white, paddingRight: 10}]}>
            {'Модель '}
          <Text
            style={{
              textTransform: 'uppercase',
            }}>
            {model}
          </Text>
        </Text>
        <Text
          numberOfLines={1}
          style={[styles.itemText, {color: R.colors.gray30}]}>
            {`Экипаж ${crew}`}
        </Text>
        <Text
          numberOfLines={1}
          style={[styles.itemText, {color: R.colors.gray30}]}>
            {`Длина ${length}`}
        </Text>
      </View>
    </TouchableOpacity>
  );
};

export const TechnologiesScreen = ({navigation}) => {
  const technologies = useSelector(state => state.technologies);
  const [error, setError] = useState(false);
  const [refreshing, setRefreshing] = useState(false);
  const {dark: isDark} = useTheme();

  const dispatch = useDispatch();

  const onRefresh = useCallback(() => {
    setRefreshing(true);
    dispatch(fetchTechnologies())
      .then(unwrapResult)
      .catch(() => setError(true))
      .finally(() => (
        setRefreshing(false),
        setError(false)
      ));
  }, []);

  useEffect(() => {
    if (!technologies.data.length) {
      onRefresh();
    }
  }, []);

  return (
    <View style={[styles.container, {backgroundColor: isDark ? R.colors.gray40 : white.background}]}>
      {error ? (
        <UnexpectedError onRefresh={onRefresh} />
      ) : (
        <FlatList
          style={{
            marginTop: 10,
          }}
          refreshControl={
            <RefreshControl onRefresh={onRefresh} refreshing={refreshing} />
          }
          data={technologies.data}
          keyExtractor={item => item.id}
          showsVerticalScrollIndicator={false}
          renderItem={({item, index}) => (
            <TechnologiesScreenItem key={index} {...item} />
          )}
        />
      )}
    </View>
  );
};

const styles = StyleSheet.create({
  container: {
    flex: 1,
    backgroundColor: '#272b30',
    paddingHorizontal: 20,
  },
  itemContent: {
    flexDirection: 'row',
    marginBottom: 10,
  },
  itemContentText: {
    marginLeft: 10,
    flex: 1,
  },
  itemText: {
    flexDirection: 'row',
    flexShrink: 1,
    fontSize: 16,
  },
});
